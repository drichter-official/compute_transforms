...
def plot_cameras(transform_matrices):
    ...
    for transform_matrix in transform_matrices:
        ...
        visualizer.extrinsic2pyramid(transform_matrix, 'c', 2.44e-3)
    plt.savefig('plenoptic_image_bsim.png',dpi=400)
    visualizer.show()
...
def calculate_angle_vectors():
    vecs = []
    for angle in [-34,0,34,-34+60,0+60,34+60,-34+2*60,0+2*60,34+2*60,-34+3*60,0+3*60,34+3*60,-34+4*60,0+4*60,34+4*60,-34+5*60,0+5*60,34+5*60]: #np.arange(0,360,60):#
