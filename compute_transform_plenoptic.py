...
def calculate_base_grid(n_img_height, n_img_width, n_focal_row, n_focal_col, pixel_size = 6.5e-3):
    size_x, size_y = pixel_size*n_img_width, pixel_size*n_img_height
    assert n_img_width % n_focal_col == 0
    assert n_img_height % n_focal_row == 0

    pixel_per_focal_x = n_img_width/n_focal_col
    pixel_per_focal_y = n_img_height/n_focal_row
    ...
    focal_points_range_x = np.linspace(-size_x/2*(1-pixel_per_focal_x/(2*n_img_width)), size_x/2*(1-pixel_per_focal_x/(2*n_img_width)), n_focal_col)
    focal_points_range_y = np.linspace(size_y/2*(1-pixel_per_focal_y/(2*n_img_height)), -size_y/2*(1-pixel_per_focal_y/(2*n_img_height)), n_focal_row)
    # Create a meshgrid
...
def compute_rays(origins, focal_vec, focal_length = 2.2):
    ...
    return rays
def calculate_camera_angle(focal_length=2.44*1e-0, sensor_width=1e-3*6.5*80):
...
def calculate_angle_vectors(angles):
    vecs = []
    for angle in angles: #np.arange(0,360,60):#
...
def main():
    ...
    EL_width = n_img_width / n_focal_col
    EL_height = n_img_height / n_focal_row

    # in mm camera parameters

    pixel_size = 6.5e-3 #* 1e-1
    focal_length = 2.44 #* 1e-1
    cam_radius = 40 #* 1e-1

    sensor_width = EL_width*pixel_size
    sensor_height = EL_height*pixel_size

    camera_angles = [-34,0,34,-34+60,0+60,34+60,-34+2*60,0+2*60,34+2*60,-34+3*60,0+3*60,34+3*60,-34+4*60,0+4*60,34+4*60,-34+5*60,0+5*60,34+5*60]

    base_names = ['0-0', '0-1', '0-2', '1-0', '1-1', '1-2', '2-0', '2-1', '2-2', '3-0', '3-1', '3-2', '4-0', '4-1', '4-2', '5-0', '5-1', '5-2']
    #base_names = np.arange(0,6)

    print('Generating base grid points...')
    grid_points_origins_base = calculate_base_grid(n_img_height,n_img_width, n_focal_row,n_focal_col,pixel_size)
    ...
    for vec,base_name in zip(angle_vecs,base_names):
        ...
        rotated_grid_points = rotation.apply(grid_points_origins_base.reshape(-1,3)).reshape(n_focal_row,n_focal_col,3)
        ...
        rays[:, :, 0] = rays[:, :, 0] + vec * cam_radius
        print(rays.shape)
        transforms, image_names = transforms_and_imagepaths(base_name,n_focal_row,n_focal_col, rays[:, :, 0], rays[:, :, 1])
        transform_list.extend(transforms)
        image_paths_list.extend(image_names)

    camera_angle_x = calculate_camera_angle(focal_length,sensor_width) # in [mm]
    camera_angle_y = calculate_camera_angle(focal_length,sensor_height)
